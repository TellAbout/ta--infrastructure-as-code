...
exports.handler = (event, context, callback) => {
    ...
    const user_role = event.request.clientMetadata.user_role;
    if(event.triggerSource === "CustomMessage_SignUp") {
        console.log('this is inside the trigger source....');        
        const link = `<a href="${verification_page_url}?username=${userName}&code=${codeParameter}&hasura_id=${hasura_id}&user_role=${user_role}" target="_blank">here</a>`;
        ...
        const link = `<a href="${forgot_password_page_url}?username=${userName}&code=${codeParameter}&hasura_id=${hasura_id}&user_role=${user_role}" target="_blank">here</a>`;
        event.response.emailSubject = "Welcome to the TellABout Service";
